<script setup lang="ts">
  ...
  import JaySecondPage from '../../components/about/JaySecondPage.vue'
  ...
  const gotoSection = (index: number, direction: number, immediate = false) => {
    ...
    if (currentIndex.value >= 0) {
      ...
      tl.to(contents[currentIndex.value]!, { yPercent: -15 * dFactor })
        .to([outer[currentIndex.value]!, inner[currentIndex.value]!], { yPercent: (i) => (i ? 100 * dFactor : -100 * dFactor) }, 0)
...
<template>
  <div>
    ...
    <div :class="['fixed w-full z-50 p-4 text-white', general.isMobile ? 'bottom-0' : 'top-0']">
      <router-link to="/">
        <n-button v-magnetic class="!text-white" size="large" quaternary :render-icon="renderIcon(HomeRound)"> 返回主页 </n-button>
      </router-link>
    </div>
    <!-- Sections -->
    ...
    <!-- Dots -->
    <div v-magnetic :class="['bg-white/40 p-2 backdrop-blur-sm rounded-full fixed right-4 z-50 flex flex-col gap-2', general.isMobile ? 'bottom-4' : 'top-1/2 -translate-y-1/2']">
      <div
