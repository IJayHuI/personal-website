<script setup>
  ...
  const props = defineProps({
    ...
    dateTextAlign: { type: String, required: false, default: 'text-left' },
    timeTextAlign: { type: String, required: false, default: 'text-left' }
  })
</script>
<template>
  <div>
    <p :class="`${props.dateTextAlign} ${props.dateTextSize}`">{{ datetime.year }}年 {{ datetime.month }}月 {{ datetime.today }}日</p>
    <p :class="`${props.dateTextAlign} ${props.dateTextSize}`">星期{{ datetime.dayOfWeek }}</p>
  </div>
  <p :class="`${props.timeTextAlign} ${props.timeTextSize}`">{{ datetime.time }}</p>
</template>
